...
x = ChIPpeakAnno::findOverlapsOfPeaks(data.peaks1, data.peaks2, data.peaks3)
pdf(file = paste0(RES, "diffbind_merged/plots/chipseeker/venn_overlaps.pdf"))
ChIPpeakAnno::makeVennDiagram(x, c("k27ac", "k27me3", "k4me3"),  fill=c("red", "blue", "green"), connectedPeaks = "keepAll")
dev.off()

saveRDS(x, paste0(RES, "diffbind_merged/overlap_significant_peaks.RData"))

overlaps <- x$peaklist[["data.peaks1///data.peaks2///data.peaks3"]]
unique <- x$uniquePeaks

overlapsAnno1 <- annotatePeak(overlaps, tssRegion=c(-1500, 1500), TxDb=txdb, annoDb="org.Hs.eg.db")
write.table(overlapsAnno1, file = paste0(RES, "diffbind_merged/annotated_peaks/overlaps_all3_annotated.csv"), sep="\t", quote=F, row.names=F)

uniqueAnno1 <- annotatePeak(unique, tssRegion=c(-1500, 1500), TxDb=txdb, annoDb="org.Hs.eg.db")
write.table(uniqueAnno1, file = paste0(RES, "diffbind_merged/annotated_peaks/unique_all3_annotated.csv"), sep="\t", quote=F, row.names=F)
